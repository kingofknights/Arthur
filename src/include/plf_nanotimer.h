...
public:
	nanotimer() { host_get_clock_service(mach_host_self(), SYSTEM_CLOCK, &system_clock); }

	~nanotimer() { mach_port_deallocate(mach_task_self(), system_clock); }
...
#if defined(__MACH__) || (defined(linux) || defined(__linux__) || defined(__linux)) || (defined(__DragonFly__) || defined(__FreeBSD__) || defined(__NetBSD__) || defined(__OpenBSD__)) || \
	defined(_WIN32)
...
inline void microsecond_delay(const double delay_us) { nanosecond_delay(delay_us * 1000.0); }

inline void millisecond_delay(const double delay_ms) { nanosecond_delay(delay_ms * 1000000.0); }
