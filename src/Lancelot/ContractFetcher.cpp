...
ContractFetcher::ContractFetcher(const std::string& name_)
    : _database(new SQLite::Database(name_, SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE)) { PrintMetaData(name_); }
...
TableWithColumnNameT ContractFetcher::GetResultWithColumnName(const std::string& query_) {
    ...
    while (query.executeStep()) {
        ...
        for (int i = 0; i < query.getColumnCount(); i++) { row.emplace(query.getColumnName(i), query.getColumn(i).getString()); }
        table.push_back(row);
    }
    return table;
}
...
TableWithColumnIndexT ContractFetcher::GetResultWithColumnIndex(const std::string& query_) {
    ...
    while (query.executeStep()) {
        ...
        for (int i = 0; i < query.getColumnCount(); i++) { row.emplace_back(query.getColumn(i).getString()); }
        table.push_back(row);
    }
    return table;
}
...
void ContractFetcher::ExecuteQuery(const std::string& query_) {
    try {
        SQLite::Statement query(*_database, query_);
        query.executeStep();
    } catch (std::exception& ex_) { LOG(ERROR, "{} {}", __FUNCTION__, ex_.what()) }
}
}// namespace Lancelot
