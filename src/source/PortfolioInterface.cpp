	...
	for (const StrategyRowPtrT& strategyRow : _strategyList) {
		...
		for (const ParameterInfoListT::value_type& value_type : strategyRow->ParameterInfoList) {
			...
			switch (parameterValue.Type) {
				case DataType_INT: {
					config["Value"] = std::to_string(parameterValue.Parameter.Integer);
					break;
				}
				case DataType_FLOAT: {
					config["Value"] = std::to_string(parameterValue.Parameter.Floating);
					break;
				}
				case DataType_RADIO: {
					config["Value"] = std::to_string(parameterValue.Parameter.Check);
					break;
				}
				case DataType_COMBO:
